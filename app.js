...
// ==================== ENCRYPTION FUNCTION ==================== //
// Yeh function normal password ko ek secret hash string me badal dega
...
async function checkSession() {
    ...
    if (savedId && savedRole) {
        ...
        } catch(err) { console.error("Session fail:", err); }
    }
}
checkSession();
...
function setLoginType(type, btn) {
    ...
    if(type === 'manager') regLink.classList.remove('hidden'); else regLink.classList.add('hidden');
}
function logout() { 
    if(confirm("Log out?")) {
        currentUser = null; localStorage.removeItem('mt_session_id'); localStorage.removeItem('mt_session_role');
        navigate('login-section'); document.getElementById('login-form').reset(); 
    }
}

// ==================== AUTH & ENCRYPTED PASSWORDS ==================== //
...
async function saveNewPassword() {
    ...
    try {
        const hashedOld = await hashPassword(oldP);
        ...
        alert("Password Encrypted & Updated Successfully!");
        closePasswordModal();
    ...
    try {
        ...
        for(let key in users) {
            if(users[key].role === 'manager' && users[key].hostel.toLowerCase() === hostelName.toLowerCase()) {
                alert(`Error: '${hostelName}' is taken.`); btn.innerText = "Register Now"; return;
            }
        }
        
        const hashedPass = await hashPassword(pass); // Password Encrypt ho raha hai
        const newMgrId = 'MGR' + Math.floor(1000 + Math.random() * 9000);
        
        users[newMgrId] = { role: 'manager', name: mgrName, hostel: hostelName, password: hashedPass };
        await saveDB('mt_users', users);
        ...
        document.getElementById('manager-register-form').reset(); navigate('login-section');
...
async function selectDate(dateStr, elem) {
    ...
    elem.classList.add('selected'); selectedDateString = dateStr;
    ...
    box.classList.remove('hidden'); document.getElementById('selected-date-title').innerText = `Meals on ${dateStr}`;
    container.innerHTML = '';
...
async function loadManagerDashboard() {
    const settings = await getDB('mt_settings'); const users = await getDB('mt_users');
    ...
    for (let key in users) {
        if (users[key].role === 'student' && users[key].hostel === currentUser.hostel) {
            tbodyManager.innerHTML += `<tr><td>${users[key].name}<br><small>${key}</small></td><td style="color:red;">₹${users[key].dues || 0}</td><td>₹0</td><td>N/A</td></tr>`;
        }
    }
}
...
document.getElementById('add-student-form').addEventListener('submit', async (e) => {
    ...
    const defaultHashedPass = await hashPassword('123'); // Student ko default Encrypted '123' milega
    
    users[newId] = { role: 'student', name, mobile, password: defaultHashedPass, firstLogin: true, dues: 0, hostel: currentUser.hostel };
    await saveDB('mt_users', users);
    document.getElementById('generated-id-msg').innerText = `ID: ${newId} (Default Pass: 123)`;
